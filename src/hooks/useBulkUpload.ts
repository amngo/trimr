...
export function useBulkUpload({
    ...
    const updateProgress = useCallback(
        (
            update:
                | Partial<BulkUploadProgress>
                | ((prev: BulkUploadProgress) => Partial<BulkUploadProgress>),
        ) => {
            setProgress((prev) => {
                const updateObj =
                ...
                if (links.length <= 100) {
                    ...
                    if (!response.ok) {
                        const errorData = await response
                            ...
                            .catch(() => ({}));
                        throw new Error(
                            errorData.error || 'Bulk upload failed',
                        );
                    }
                    ...
                    if (result.successful > 0 && result.failed === 0) {
                        toast.success(
                            `Successfully created ${result.successful} links!`,
                        );
                    } else if (result.successful > 0 && result.failed > 0) {
                        toast.warning(
                            `Created ${result.successful} links, ${result.failed} failed`,
                        );
                    } else {
                        toast.error(
                            'All uploads failed. Please check your CSV data.',
                        );
                    }
                        ...
                        const batchPromises = batch.map(
                            async (linkData, batchIndex) => {
                                ...
                                try {
                                    ...
                                    const result =
                                        await createSingleLink(linkData);
...
async function createSingleLink(linkData: CSVLinkData): Promise<{
    ...
    try {
        ...
        if (linkData.password) {
            formData.append('password', linkData.password);
        }
